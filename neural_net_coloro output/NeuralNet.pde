  ...
  void fillNet(){
   ...
   void show(float x, float y, float size, boolean showParameter){
    ...
    // print("edges: "+edges.size());
      ...
      float one = size/(layers.size()-1.0);
     for(int i = 0; i<layers.size()-1; i++){
       float two = size/(layers.get(i).length+1.0);
       float three = size/(layers.get(i+1).length+1.0);
       for(int j = 0; j<layers.get(i).length; j++){
         for(int k = 0; k<layers.get(i+1).length; k++){
           edges.get(i)[j][k].show(x+((one)*i), y+(two*(j+1.0)), x+((one)*(i+1.0)), y+((three)*(k+1.0)));
         }    
       }
     }
     for(int i = 0; i<layers.size(); i++){
       for(int j = 0; j<layers.get(i).length; j++){
         layers.get(i)[j].show(x+((one)*i), y+((size/(layers.get(i).length+1))*(j+1.0)));
       }
     }
